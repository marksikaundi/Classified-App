        ...
        {navigationLinks.map((link) => (
            <Link key={link.id} href={link.href}>
                <div className="group relative hover:bg-sky-500 transition-shadow">
                    <li className="block text-white hover:text-gray-600 p-2">{link.text}</li>
                </div>
            </Link>
        ))}
    </nav>
...
<div className={nav ? 'md:hidden absolute z-10 left-0 right-0 top-24 h-80 bottom-0 w-full bg-gray-200 items-center duration-300' : 'md:hidden absolute z-10 top-[-100%] right-0 h-80 bottom-0 w-full bg-gray-200 flex justify-center items-center duration-300'}>
